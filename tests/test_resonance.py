...
def spirit(db_session):
    ...
    db_session.commit()
    yield row
    db_session.query(models.ResonanceEvent).filter_by(spirit_id=row.place_id).delete()
    db_session.query(models.Resonance).filter_by(spirit_id=row.place_id).delete()
    db_session.commit()
...
def test_first_award_creates_row(db_session, player, spirit):
    result = _award(db_session, player, spirit.place_id, amount=AMOUNT_QUEST)
    ...
    row = db_session.query(models.Resonance).filter_by(
        player_id=player, spirit_id=spirit.place_id
    ).one()
...
def test_awards_accumulate(db_session, player, spirit):
    _award(db_session, player, spirit.place_id, amount=AMOUNT_QUEST)
    result = _award(db_session, player, spirit.place_id, amount=AMOUNT_QUEST)
...
def test_event_ledger_records_each_award(db_session, player, spirit):
    _award(db_session, player, spirit.place_id, source_id="quest-a")
    _award(db_session, player, spirit.place_id, source_id="quest-b")
...
def test_same_source_is_not_credited_twice(db_session, player, spirit):
    ...
    first = _award(db_session, player, spirit.place_id, source_id="quest-001")
    second = _award(db_session, player, spirit.place_id, source_id="quest-001")
...
def test_duplicate_does_not_raise(db_session, player, spirit):
    ...
    （網路重試、連點兩下），不是錯誤。
    """
    _award(db_session, player, spirit.place_id, source_id="quest-001")
    _award(db_session, player, spirit.place_id, source_id="quest-001")  # 不該拋
...
def test_session_still_usable_after_duplicate(db_session, player, spirit):
    ...
    """
    _award(db_session, player, spirit.place_id, source_id="quest-001", amount=AMOUNT_QUEST)
    _award(db_session, player, spirit.place_id, source_id="quest-001", amount=AMOUNT_QUEST)

    result = _award(db_session, player, spirit.place_id, source_id="quest-002", amount=AMOUNT_QUEST)
...
def test_same_source_id_different_source_type_is_a_separate_award(db_session, player, spirit):
    ...
    first = _award(
        db_session, player, spirit.place_id, source_type=SOURCE_QUEST, source_id="X", amount=20
    )
    second = _award(
        db_session,
        player,
...
def test_same_source_is_blocked_across_spirits(db_session, player, spirit, client):
    ...
    try:
        first = _award(db_session, player, spirit.place_id, source_id="shared-id")
        second = _award(db_session, player, other.place_id, source_id="shared-id")

        assert first.awarded is True
        assert second.awarded is False
    finally:
        db_session.query(models.ResonanceEvent).filter_by(spirit_id=other.place_id).delete()
        db_session.query(models.Resonance).filter_by(spirit_id=other.place_id).delete()
        db_session.commit()
...
def test_crossing_first_threshold_reports_unlock(db_session, player, spirit):
    """0 → 10：跨過第一個門檻。"""
    result = _award(
        db_session, player, spirit.place_id, amount=AMOUNT_ENCOUNTER_COLLECTION
    )
...
def test_not_crossing_a_threshold_reports_no_unlock(db_session, player, spirit):
    ...
    """
    _award(db_session, player, spirit.place_id, amount=10, source_id="a")
    result = _award(db_session, player, spirit.place_id, amount=20, source_id="b")
...
def test_below_first_threshold_reports_no_unlock(db_session, player, spirit):
    result = _award(db_session, player, spirit.place_id, amount=5)
...
def test_crossing_second_threshold(db_session, player, spirit):
    """30 → 50：跨過 40。"""
    _award(db_session, player, spirit.place_id, amount=30, source_id="a")
    result = _award(db_session, player, spirit.place_id, amount=20, source_id="b")
...
def test_crossing_multiple_thresholds_reports_each(db_session, player, spirit):
    ...
    """
    _award(db_session, player, spirit.place_id, amount=5, source_id="a")
    result = _award(db_session, player, spirit.place_id, amount=50, source_id="b")
...
def test_duplicate_award_reports_no_unlock(db_session, player, spirit):
    """重複入帳沒有加值，自然也不該回報解鎖。"""
    first = _award(db_session, player, spirit.place_id, amount=10, source_id="dup")
    assert first.newly_unlocked_stages == [1]

    second = _award(db_session, player, spirit.place_id, amount=10, source_id="dup")
...
def test_quest_awards_twenty_per_distinct_quest(db_session, player, spirit):
    ...
    _award(db_session, player, spirit.place_id, source_type=SOURCE_QUEST, source_id="q1", amount=AMOUNT_QUEST)
    result = _award(
        db_session, player, spirit.place_id, source_type=SOURCE_QUEST, source_id="q2", amount=AMOUNT_QUEST
    )
...
def test_resonance_is_per_player(db_session, player, spirit, client):
    ...
    _award(db_session, player, spirit.place_id, amount=20)
    result = _award(db_session, other_player, spirit.place_id, amount=20)
...
def test_resonance_is_per_spirit(db_session, player, spirit):
    ...
    try:
        _award(db_session, player, spirit.place_id, amount=20, source_id="a")
        result = _award(db_session, player, other.place_id, amount=20, source_id="b")

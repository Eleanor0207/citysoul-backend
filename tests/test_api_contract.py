...
EXPECTED_RESPONSE_CODES = {
    ...
    # 道具查詢（待辦 P3 第 21 項）。沒有 404：一件都沒有時回空陣列，那是正常的
    # 起始狀態。也沒有 422，因為它不吃參數——玩家從 token 解出來。
    ("get", "/api/v1/inventory"): {"200", "401"},
    # backend#75「當地氛圍」。503 是這支唯一會回的失敗碼，而且有兩種意思：
    # 這個環境沒設金鑰，或上游查詢失敗——兩者對客戶端是同一件事（整條 bar 隱藏），
    # 所以刻意不分開。沒有 403：它不看距離也不看憑證持有人。
    ("get", "/api/v1/weather/current"): {"200", "401", "422", "503"},

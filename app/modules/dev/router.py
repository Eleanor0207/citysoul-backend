...
也不該有「列出所有玩家」。測試的人要能在幾個身分之間切換來看共鳴值差異，
手上沒有那份清單就只能自己記 device_id。

這支的存在理由是「測試時人要看得到有什麼」，不是「產品缺了這個功能」。
不要因為主控台有，就把它搬進 `/api/v1`。

⚠️ 地標清單**曾經**也是這一類，理由是「客戶端是從地圖上點選的」。那個理由的
前提是客戶端自己手上有清單（打包在 `tiles/near/index.json` 裡）。近景層改走
即時 mesh 之後那份索引退場，前提消失，所以 `GET /api/v1/spirits` 在
2026-08-15 成為正式 API。`/dev/spirits` 留著，因為它跟正式那支行為不同：
**下架的靈魂也會出現**，測試的人需要看得到「為什麼這個地標打 404」。
"""
